...
#if defined(_WIN32)
// Winsock2 must be included before the MSVC system headers; we do not use
// winpthread here, so keep the raw socket layer mapped 1:1 onto Winsock.
...
#endif
static int sockErrno() { return WSAGetLastError(); }
static bool sockInvalid(int fd) { return fd == int(INVALID_SOCKET); }
static void sockClose(int fd) { closesocket(fd); }
static int sockInitOnce()
{
...
#include <arpa/inet.h>
#include <unistd.h>
static int sockErrno() { return errno; }
static bool sockInvalid(int fd) { return fd < 0; }
static void sockClose(int fd) { ::close(fd); }
static int sockInitOnce() { return 0; }
#endif
...
static int cryptoSecretboxKeygen(lua_State* L) { std::string key(crypto_secretbox_KEYBYTES, '\0'); randombytes_buf(key.data(), key.size()); lua_pushlstring(L,key.data(),key.size()); return 1; }
static int cryptoSecretboxSeal(lua_State* L) { std::string msg=checkString(L,1), nonce=checkString(L,2), key=checkString(L,3); if(nonce.size()!=crypto_secretbox_NONCEBYTES||key.size()!=crypto_secretbox_KEYBYTES) return luaL_error(L,"invalid secretbox nonce or key length"); std::string out(msg.size()+crypto_secretbox_MACBYTES,'\0'); crypto_secretbox_easy((unsigned char*)out.data(),(const unsigned char*)msg.data(),msg.size(),(const unsigned char*)nonce.data(),(const unsigned char*)key.data()); lua_pushlstring(L,out.data(),out.size()); return 1; }
static int cryptoSecretboxOpen(lua_State* L) { std::string box=checkString(L,1), nonce=checkString(L,2), key=checkString(L,3); if(nonce.size()!=crypto_secretbox_NONCEBYTES||key.size()!=crypto_secretbox_KEYBYTES||box.size()<crypto_secretbox_MACBYTES) return luaL_error(L,"invalid secretbox input"); std::string out(box.size()-crypto_secretbox_MACBYTES,'\0'); if(crypto_secretbox_open_easy((unsigned char*)out.data(),(const unsigned char*)box.data(),box.size(),(const unsigned char*)nonce.data(),(const unsigned char*)key.data())!=0){lua_pushnil(L);lua_pushliteral(L,"authentication failed");return 2;} lua_pushlstring(L,out.data(),out.size()); return 1; }
static int cryptoPasswordHash(lua_State* L) { std::string password=checkString(L,1); char out[crypto_pwhash_STRBYTES]; if(crypto_pwhash_str(out,password.data(),password.size(),crypto_pwhash_OPSLIMIT_MODERATE,crypto_pwhash_MEMLIMIT_MODERATE)!=0)return luaL_error(L,"password hashing failed"); lua_pushstring(L,out); return 1; }
static int cryptoPasswordVerify(lua_State* L) { std::string password=checkString(L,1), encoded=checkString(L,2); lua_pushboolean(L,crypto_pwhash_str_verify(encoded.c_str(),password.data(),password.size())==0); return 1; }
static int cryptoOpen(lua_State* L) { lua_newtable(L); int t=lua_gettop(L); setfn(L,t,"digest",cryptoDigest); setfn(L,t,"hash",cryptoDigest); setfn(L,t,"hmac",cryptoHmac); setfn(L,t,"randomBytes",cryptoRandom); lua_newtable(L); int b=lua_gettop(L); setfn(L,b,"encode",cryptoBase64Encode); setfn(L,b,"decode",cryptoBase64Decode); lua_setfield(L,t,"base64"); lua_newtable(L); int s=lua_gettop(L); setfn(L,s,"keygen",cryptoSecretboxKeygen); setfn(L,s,"seal",cryptoSecretboxSeal); setfn(L,s,"open",cryptoSecretboxOpen); lua_pushinteger(L,crypto_secretbox_NONCEBYTES); lua_setfield(L,s,"nonceBytes"); lua_pushinteger(L,crypto_secretbox_KEYBYTES); lua_setfield(L,s,"keyBytes"); lua_setfield(L,t,"secretbox"); lua_newtable(L); int p=lua_gettop(L); setfn(L,p,"hash",cryptoPasswordHash); setfn(L,p,"verify",cryptoPasswordVerify); lua_setfield(L,t,"password"); return 1; }
...
static int socketClose(lua_State* L){auto*s=checkSocket(L,1); if(!sockInvalid(s->fd)&&s->fd>=0){sockClose(s->fd);s->fd=-1;} return 0;}
static int socketSend(lua_State* L){auto*s=checkSocket(L,1); std::string data=checkString(L,2); if(s->fd<0)return luaL_error(L,"socket closed"); if(!s->websocket){ssize_t n=send(s->fd,data.data(),data.size(),0);if(n<0)return luaL_error(L,"send failed");lua_pushinteger(L,n);return 1;} bool masked=!s->serverSide; std::vector<unsigned char> frame; frame.push_back(0x81); unsigned char mask[4]={0x13,0x37,0x42,0x99}; if(data.size()<126)frame.push_back((unsigned char)data.size()|(masked?0x80:0)); else if(data.size()<=65535){frame.push_back(126|(masked?0x80:0));frame.push_back(data.size()>>8);frame.push_back(data.size());} else return luaL_error(L,"message too large"); if(masked)frame.insert(frame.end(),mask,mask+4); for(size_t i=0;i<data.size();i++)frame.push_back((unsigned char)data[i]^(masked?mask[i%4]:0)); if(send(s->fd,(const char*)frame.data(),frame.size(),0)<0)return luaL_error(L,"send failed"); return 0;}
...
static int netConnect(lua_State* L){if(sockInitOnce()!=0)return luaL_error(L,"socket subsystem initialization failed");std::string host=checkString(L,1);int port=luaL_checkinteger(L,2);int fd=socket(AF_INET,SOCK_STREAM,0);if(sockInvalid(fd))return luaL_error(L,"socket failed");sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(port);if(inet_pton(AF_INET,host.c_str(),&a.sin_addr)!=1){sockClose(fd);return luaL_error(L,"invalid IPv4 address");}if(connect(fd,(sockaddr*)&a,sizeof(a))<0){sockClose(fd);return luaL_error(L,"connect failed: %s",strerror(sockErrno()));}auto*s=(Socket*)lua_newuserdata(L,sizeof(Socket));*s={fd,false,false};luaL_getmetatable(L,"Lumora.Socket");lua_setmetatable(L,-2);return 1;}
...
static int netWebsocketConnect(lua_State* L){std::string url=checkString(L,1);if(url.rfind("ws://",0)!=0)return luaL_error(L,"only ws:// is supported");std::string rest=url.substr(5),hostport=rest,path="/";size_t slash=rest.find('/');if(slash!=std::string::npos){hostport=rest.substr(0,slash);path=rest.substr(slash);}size_t colon=hostport.rfind(':');if(colon==std::string::npos)return luaL_error(L,"WebSocket URL needs a port");std::string host=hostport.substr(0,colon);int port=std::stoi(hostport.substr(colon+1));if(sockInitOnce()!=0)return luaL_error(L,"socket subsystem initialization failed");int fd=socket(AF_INET,SOCK_STREAM,0);if(sockInvalid(fd))return luaL_error(L,"socket failed");sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(port);if(inet_pton(AF_INET,host.c_str(),&a.sin_addr)!=1||connect(fd,(sockaddr*)&a,sizeof(a))<0){sockClose(fd);return luaL_error(L,"WebSocket connect failed");}std::string key="bXlzdGF0aWNrLWtleQ==";std::string request="GET "+path+" HTTP/1.1\r\nHost: "+hostport+"\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: "+key+"\r\nSec-WebSocket-Version: 13\r\n\r\n";send(fd,request.data(),request.size(),0);char response[4096];ssize_t n=recv(fd,response,sizeof(response)-1,0);if(n<0||std::string(response,n).find("101") == std::string::npos){sockClose(fd);return luaL_error(L,"WebSocket handshake failed");}auto*s=(Socket*)lua_newuserdata(L,sizeof(Socket));*s={fd,true,false};luaL_getmetatable(L,"Lumora.Socket");lua_setmetatable(L,-2);return 1;}
static int netServerListen(lua_State* L){if(sockInitOnce()!=0)return luaL_error(L,"socket subsystem initialization failed");std::string host=optString(L,1,"0.0.0.0");int port=luaL_checkinteger(L,2);luaL_checktype(L,3,LUA_TFUNCTION);int fd=socket(AF_INET,SOCK_STREAM,0);if(sockInvalid(fd))return luaL_error(L,"socket failed");int yes=1;setsockopt(fd,SOL_SOCKET,SO_REUSEADDR,(const char*)&yes,sizeof(yes));sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(port);a.sin_addr.s_addr=host=="0.0.0.0"?INADDR_ANY:inet_addr(host.c_str());if(bind(fd,(sockaddr*)&a,sizeof(a))<0||listen(fd,8)<0){sockClose(fd);return luaL_error(L,"listen failed: %s",strerror(sockErrno()));}int client=accept(fd,nullptr,nullptr);sockClose(fd);if(sockInvalid(client))return luaL_error(L,"accept failed");char buf[65536];ssize_t n=recv(client,buf,sizeof(buf)-1,0);if(n<0){sockClose(client);return luaL_error(L,"receive failed");}buf[n]=0;lua_newtable(L);lua_pushlstring(L,buf,n);lua_setfield(L,-2,"raw");lua_pushliteral(L,"GET");lua_setfield(L,-2,"method");lua_pushliteral(L,"/");lua_setfield(L,-2,"path");lua_pushvalue(L,3);lua_insert(L,-2);if(lua_pcall(L,1,1,0)!=0){sockClose(client);lua_error(L);return 0;}std::string response=lua_isstring(L,-1)?checkString(L,-1):"";lua_pop(L,1);std::string out="HTTP/1.1 200 OK\r\nContent-Length: "+std::to_string(response.size())+"\r\nConnection: close\r\n\r\n"+response;send(client,out.data(),out.size(),0);sockClose(client);lua_pushboolean(L,1);return 1;}
static int netServerWebSocket(lua_State* L){if(sockInitOnce()!=0)return luaL_error(L,"socket subsystem initialization failed");std::string host=optString(L,1,"0.0.0.0");int port=luaL_checkinteger(L,2);luaL_checktype(L,3,LUA_TFUNCTION);int fd=socket(AF_INET,SOCK_STREAM,0);if(sockInvalid(fd))return luaL_error(L,"socket failed");int yes=1;setsockopt(fd,SOL_SOCKET,SO_REUSEADDR,(const char*)&yes,sizeof(yes));sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(port);a.sin_addr.s_addr=host=="0.0.0.0"?INADDR_ANY:inet_addr(host.c_str());if(bind(fd,(sockaddr*)&a,sizeof(a))<0||listen(fd,8)<0){sockClose(fd);return luaL_error(L,"listen failed: %s",strerror(sockErrno()));}int client=accept(fd,nullptr,nullptr);sockClose(fd);if(sockInvalid(client))return luaL_error(L,"accept failed");char buf[8192];ssize_t n=recv(client,buf,sizeof(buf)-1,0);if(n<=0){sockClose(client);return luaL_error(L,"handshake receive failed");}std::string request(buf,n),key="";size_t p=request.find("Sec-WebSocket-Key:");if(p!=std::string::npos){p+=19;while(p<request.size()&&request[p]==' ')p++;size_t e=request.find("\r\n",p);key=request.substr(p,e-p);}if(key.empty()){sockClose(client);return luaL_error(L,"missing WebSocket key");}std::string response="HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: "+wsAccept(key)+"\r\n\r\n";send(client,response.data(),response.size(),0);auto*s=(Socket*)lua_newuserdata(L,sizeof(Socket));*s={client,true,true};luaL_getmetatable(L,"Lumora.Socket");lua_setmetatable(L,-2);lua_pushvalue(L,3);lua_insert(L,-2);if(lua_pcall(L,1,0,0)!=0){sockClose(client);lua_error(L);return 0;}lua_pushboolean(L,1);return 1;}
