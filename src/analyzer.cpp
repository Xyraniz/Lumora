...
int runAnalyzerCommand(const std::string& command,const AnalyzerOptions& options){try{Result r=analyze(options.input);if(command=="inspect"||command=="report"||options.json){std::cout<<reportJson(r)<<"\n";}if(command=="deobfuscate"){std::filesystem::path out;if(options.outputDir.empty())out=std::filesystem::path(options.input).parent_path()/"lumora-analysis";else out=options.outputDir;std::filesystem::create_directories(out);writeText(out/"00-original.luau",r.original);writeText(out/"01-strings-decoded.luau",r.strings);writeText(out/"02-constants-folded.luau",r.constants);writeText(out/"03-symbols-renamed.luau",r.normalized);writeText(out/"04-reconstructed.luau",r.reconstructed);writeText(out/"diff.json",diffJson(r.original,r.reconstructed));writeText(out/"report.json",reportJson(r));if(!options.json)std::cout<<"analysis written to "<<out.string()<<"\n";}return 0;}catch(const std::exception&e){std::cerr<<"analysis error: "<<e.what()<<"\n";return 2;}}
